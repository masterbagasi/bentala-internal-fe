...
export default function AccessControlClient() {
  ...
  useEffect(() => {
    ...
    fetch('/api/access')
      ...
      .then((data: { users: AccessUser[]; sections: SectionMeta[] }) => {
        if (cancelled) return
        setSections(data.sections)
        setUsers(data.users)
        const d: Record<string, Set<string>> = {}
        for (const u of data.users) d[u.email] = new Set(u.sections)
        setDraft(d)
        setSaved(cloneMap(d))
        setLoading(false)
      })
      .catch(() => {
        if (cancelled) return
        setLoadError('Gagal memuat data akses. Pastikan Anda super admin.')
        setLoading(false)
      })
    return () => {
      cancelled = true
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
        <div style={{ fontSize: 12, color: 'var(--text2)', maxWidth: 560, lineHeight: 1.5 }}>
